...
import * as companyPhotos from "./functions/api/company-photos/index";
import * as companyPhotoById from "./functions/api/company-photos/[id]";
...
export default {
  async fetch(
    ...
    if (path === "/api/content") {
      if (method === "OPTIONS" && content.onRequestOptions) {
        return callHandler(content.onRequestOptions, request, env);
      }
      ...
      if (method === "DELETE" && content.onRequestDelete) {
        return callHandler(content.onRequestDelete, request, env);
      }
    ...
    // =========================
    // COMPANY PHOTOS (FACTORY & OFFICE)
    // =========================

    if (path === "/api/company-photos" || path === "/api/gallery") {

      if (method === "GET" && companyPhotos.onRequestGet) {
        return callHandler(
          companyPhotos.onRequestGet,
          request,
          env
        );
      }

      if (method === "POST" && companyPhotos.onRequestPost) {
        return callHandler(
          companyPhotos.onRequestPost,
          request,
          env
        );
      }
    ...
    if (path.startsWith("/api/company-photos/") || path.startsWith("/api/gallery/")) {
      const prefix = path.startsWith("/api/company-photos/") ? "/api/company-photos/" : "/api/gallery/";
      const id = decodeURIComponent(path.substring(prefix.length));

      if (id) {
        if (method === "DELETE" && companyPhotoById.onRequestDelete) {
          return callHandler(
            companyPhotoById.onRequestDelete,
            request,
            env,
            { id }
          );
        ...
        if ((method === "PUT" || method === "POST") && companyPhotoById.onRequestPut) {
          return callHandler(
            companyPhotoById.onRequestPut,
            request,
            env,
            { id }
          );
