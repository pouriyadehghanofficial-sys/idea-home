...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#EDEAE4]/50 flex flex-col font-vazir text-[#1E4B57]">
      ...
      <header className="bg-[#1E4B57]/90 backdrop-blur-xl text-white border-b border-white/15 sticky top-0 z-40 shadow-sm">
        <div className={`${isVisualEditor ? 'max-w-[1800px]' : 'max-w-7xl'} mx-auto px-4 sm:px-6 lg:px-8`}>
          <div className="flex justify-between items-center h-16 sm:h-20">
            ...
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <button
                ...
                className="lg:hidden w-10 h-10 rounded-2xl bg-white/10 hover:bg-white/20 active:bg-white/30 text-white backdrop-blur-md transition-all flex items-center justify-center shrink-0 cursor-pointer touch-manipulation shadow-xs"
              ...
              <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                <div className="bg-white/10 backdrop-blur-md p-1.5 rounded-2xl border border-white/20 shadow-xs shrink-0">
                  <Logo variant="header" showText={false} />
                </div>
                <div className="min-w-0">
                  <h1 className="font-black text-xs sm:text-base text-[#EDEAE4] flex items-center gap-1.5 sm:gap-2 whitespace-nowrap overflow-hidden">
                    <span className="hidden md:inline whitespace-nowrap">پنل مدیریت کارخانه ایده استیل سازان شریف</span>
                    <span className="md:hidden whitespace-nowrap font-black">پنل مدیریت</span>
                  </h1>
                </div>
              </div>
            </div>

            {/* Quick Tab Navigation on Header (Desktop & Large Tablet) */}
            <div className="hidden md:flex items-center gap-1.5 bg-white/10 backdrop-blur-md p-1 rounded-2xl border border-white/15">
              {menuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                      isActive
                        ? 'bg-[#C9A24B] text-[#1E4B57] shadow-sm'
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
              <button
                ...
                className="text-xs bg-white/10 hover:bg-white/20 text-[#EDEAE4] px-2.5 sm:px-3.5 py-2 rounded-2xl flex items-center gap-1.5 transition-all border border-white/15 backdrop-blur-md shadow-xs cursor-pointer active:scale-98 shrink-0 whitespace-nowrap"
                title="مشاهده ظاهر عمومی سایت"
              >
                <ExternalLink className="w-3.5 h-3.5 text-[#C9A24B]" />
                <span className="hidden sm:inline font-bold">مشاهده وب‌سایت</span>
              </button>

              <button
                type="button"
                onClick={onLogout}
                className="text-xs bg-red-600/80 hover:bg-red-600 text-white px-2.5 sm:px-3.5 py-2 rounded-2xl flex items-center gap-1.5 transition-all shadow-xs border border-red-400/20 backdrop-blur-md cursor-pointer active:scale-98 shrink-0 whitespace-nowrap"
      ...
      {/* Global Mobile/Tablet Navigation Drawer (available across all tabs including visual editor, slider, and catalog) */}
      <AnimatePresence>
        {mobileOpen && (
          <>
            {/* Backdrop */}
            ...
            <motion.div
              ...
              className="fixed top-16 sm:top-20 inset-x-0 mx-auto w-[calc(100%-2rem)] max-w-md p-2 sm:p-4 z-50 lg:hidden"
              dir="rtl"
            >
        ...
        <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <aside className="hidden lg:block lg:col-span-3 space-y-4">
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-4 shadow-sm border border-white/80 space-y-2 sticky top-28">
              ...
              {/* User Profile Card */}
              <div className="p-3.5 rounded-2xl bg-gradient-to-br from-[#1E4B57]/5 to-[#346D80]/10 border border-[#7FA69C]/25 flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#1E4B57] text-[#C9A24B] flex items-center justify-center shadow-xs">
                  <ShieldCheck className="w-5 h-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <span className="text-xs font-black text-[#1E4B57] block truncate">آیدیا هوم</span>
                </div>
              </div>
