...
export const AdminSlider: React.FC<AdminSliderProps> = ({ items: initialItems, onSave }) => {
  ...
  const handleDelete = (index: number) => {
    if (items.length <= 1) {
      setErrorMessage('حداقل یک تصویر باید در اسلایدر باقی بماند.');
  ...
  const confirmDeleteSlide = () => {
    ...
    const updated = items.filter((_, i) => i !== slideToDelete);
    setItems(updated);
  ...
  return (
    <div className="space-y-6" dir="rtl">
          ...
          <div className="bg-[#EDEAE4] rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl border border-red-200 text-center animate-in zoom-in-95 duration-150">
            ...
            <div className="space-y-1.5">
              <h3 className="font-black text-base text-[#1E4B57]">
                آیا از حذف اسلاید {slideToDelete + 1} اطمینان دارید؟
              </h3>
              <p className="text-xs text-[#1E4B57]/70 leading-relaxed">
                این عکس از چرخه نمایش اسلایدر صفحه اصلی حذف خواهد شد.
              </p>
            </div>
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
                ...
                className="px-4 py-2.5 text-xs font-bold text-[#1E4B57] hover:bg-white/80 rounded-2xl border border-[#1E4B57]/20 transition-colors cursor-pointer"
              >
                انصراف
              </button>
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-5 py-2.5 rounded-2xl shadow-md transition-colors cursor-pointer flex items-center gap-1.5 active:scale-95"
              >
                <Trash2 className="w-3.5 h-3.5" />
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-[#EDEAE4] rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl border border-[#C9A24B]/30 text-center animate-in zoom-in-95 duration-150">
            <div className="w-12 h-12 rounded-2xl bg-[#C9A24B]/20 text-[#1E4B57] mx-auto flex items-center justify-center shadow-xs">
              <AlertCircle className="w-6 h-6 text-[#C9A24B]" />
            </div>
            <div className="space-y-1.5">
              <h3 className="font-black text-base text-[#1E4B57]">
                بازنشانی به ۱۰ اسلاید پیش‌فرض؟
              </h3>
              <p className="text-xs text-[#1E4B57]/70 leading-relaxed">
                تمام اسلایدها به تنظیمات اولیه کارخانه و عکس‌های استاندارد بازگردانده خواهند شد.
              </p>
            </div>
            <div className="flex items-center justify-center gap-3 pt-2">
              <button
              ...
              <button
                ...
                className="bg-[#C9A24B] hover:bg-[#b58f3d] text-[#1E4B57] text-xs font-bold px-5 py-2.5 rounded-2xl shadow-md transition-colors cursor-pointer flex items-center gap-1.5 active:scale-95"
              >
                <span>تایید و بازنشانی</span>
              </button>
            </div>
          </div>
        </div>
