...
import { Product, CatalogInfo, ContactMessage, AdminUser, Category, SliderProduct } from '../types';
import { INITIAL_PRODUCTS, INITIAL_CATEGORIES, INITIAL_CATALOG, INITIAL_MESSAGES } from '../data/initialData';
...
class StorageService {
  ...
  async getProducts(): Promise<Product[]> {
    try {
      ...
      if (res.ok && contentType.includes('application/json')) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          await idbSet(LOCAL_PRODUCTS_KEY, data);
          safeLocalStorageSet(LOCAL_PRODUCTS_KEY, JSON.stringify(data));
          return data;
        }
      }
    } catch {
    ...
    if (Array.isArray(fromIdb) && fromIdb.length > 0) {
      safeLocalStorageSet(LOCAL_PRODUCTS_KEY, JSON.stringify(fromIdb));
      return fromIdb;
    }
    ...
    if (local) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          await idbSet(LOCAL_PRODUCTS_KEY, parsed);
          return parsed;
        }
      } catch {}
    }

    await idbSet(LOCAL_PRODUCTS_KEY, INITIAL_PRODUCTS);
  ...
  async addCategory(name: string): Promise<Category> {
    ...
    if (!res.ok) {
      const errData = await res.json().catch(() => ({ error: 'خطا در افزودن دسته‌بندی به سرور' }));
      throw new Error(errData.error || errData.message || 'خطا در افزودن دسته‌بندی روی سرور');
    }

    const savedCat = await res.json();
    const all = await this.getCategories().catch(() => []);
    const updated = [...all.filter((c) => c.name !== savedCat.name), savedCat];
  ...
  async deleteCategory(id: string): Promise<boolean> {
    const res = await this.fetchWithAuth(`/api/categories?id=${encodeURIComponent(id)}`, {
      method: 'DELETE'
    });

    if (!res.ok) {
      const errData = await res.json().catch(() => ({ error: 'خطا در حذف دسته‌بندی از سرور' }));
      throw new Error(errData.error || errData.message || 'خطا در حذف دسته‌بندی از سرور');
    }
  ...
  async getSliderProducts(): Promise<SliderProduct[]> {
    ...
    try {
      const res = await fetch('/api/slider?all=true');
      const contentType = res.headers.get('content-type') || '';
      if (res.ok && contentType.includes('application/json')) {
        const data = await res.json();
        if (Array.isArray(data) && data.length > 0) {
  ...
  async uploadFile(
    file: File,
    folder: 'products' | 'catalog' | 'slider' | 'uploads' = 'products',
    extra?: Record<string, string>
  ): Promise<{ success: boolean; url: string; key?: string; publicId?: string; fileId?: string; error?: string }> {
    let uploadTargetFile = file;
    let fallbackDataUrl = '';

    // Catalog PDF: upload through Cloudflare server -> ImageKit
    // This avoids direct browser access to upload.imagekit.io.
    if (folder === 'catalog' || file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
      ...
      try {
        ...
        formData.append('fileName', file.name || 'IdeaHome-Catalog.pdf');
