...
import { X, CheckCircle2, FileCheck, Phone, Send, Sparkles, Building, User, Mail, MessageSquare } from 'lucide-react';
...
export const OrderModal: React.FC<OrderModalProps> = ({ product, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (product) {
      setNotes(`درخواست استعلام قیمت و کاتالوگ برای محصول: ${product.title}`);
    } else {
      setNotes('');
    }
  }, [product]);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {product && (
                <div className="p-3.5 rounded-xl bg-[#EDEAE4]/[0.06] border border-[#C9A24B]/30 flex items-center justify-between">
                  <div>
                    <span className="text-[11px] text-[#7FA69C] block font-vazir">کالای انتخاب‌شده:</span>
                    <span className="text-xs font-bold text-[#EDEAE4] font-vazir">{product.title}</span>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  <label className="block text-xs font-bold text-[#EDEAE4]/80 mb-1.5 font-vazir">
                    نام و نام خانوادگی <span className="text-[#C9A24B]">*</span>
                  </label>
                ...
                <div>
                  <label className="block text-xs font-bold text-[#EDEAE4]/80 mb-1.5 font-vazir">
                    شماره همراه (واتساپ / تماس) <span className="text-[#C9A24B]">*</span>
                  </label>
                  <input
                    type="tel"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                <div>
                  ...
                  >
                    <option value="سفارش عمده">سفارش عمده</option>
                    <option value="دریافت کاتالوگ و لیست قیمت روز">دریافت کاتالوگ و لیست قیمت روز</option>
                    <option value="اخذ نمایندگی پخش استانی">اخذ نمایندگی پخش استانی</option>
                    <option value="صادرات و بازرگانی خارجی">صادرات و بازرگانی خارجی</option>
                  </select>
                </div>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>{submitting ? 'در حال ارسال اطلاعات...' : 'ارسال درخواست استعلام قیمت'}</span>
                </button>
              </div>
            </form>
          )}
