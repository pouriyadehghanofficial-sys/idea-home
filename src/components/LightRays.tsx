...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  return m ? [parseInt(m[1], 16) / 255, parseInt(m[2], 16) / 255, parseInt(m[3], 16) / 255] : [0.788, 0.635, 0.294];
};
...
export function LightRays({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      const targetDpr = isMobileDevice ? 1 : Math.min(window.devicePixelRatio || 1, 1.5);
