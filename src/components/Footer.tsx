...
export const Footer: React.FC<FooterProps> = ({ onOpenOrderModal }) => {
  ...
  const { isEditorMode, language } = useSiteContent();

  const creditPrefixText = language === 'en' 
    ? 'Designed and Developed by' 
    : language === 'ar' 
    ? 'تصميم وتطوير بواسطة' 
    : 'طراحی و توسعه توسط';

  const creditCompanyText = language === 'en'
    ? 'Ahoora Smart Pioneers Co.'
    : language === 'ar'
    ? 'شركة أهورا للرواد الأذكياء'
    : 'شرکت نوآوران هوشمند اهورا';

  return (
    <footer id="contact" className="bg-[#1E4B57] text-[#EDEAE4] pt-12 sm:pt-16 pb-12 border-t border-[#EDEAE4]/10 relative overflow-hidden scroll-mt-16 sm:scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-8 sm:gap-14 mb-12 sm:mb-14 items-start`}>
          ...
          <div className="space-y-3.5 text-start md:justify-self-end md:mr-auto w-full md:max-w-md">
            <EditableText
              ...
              <li className="flex items-center gap-2.5">
                ...
                <span className="font-vazir text-xs text-[#EDEAE4]/90 flex items-center gap-2 flex-wrap" dir="rtl">
                  <EditableText
                    ...
                    as="span"
                    className="font-bold text-[#EDEAE4]"
                    defaultText="طاهری:"
                  />
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-3 sm:gap-4 text-[11px]">
            ...
            >
              <EditableText
                id="footer.backToTop"
                as="span"
                defaultText="بازگشت به بالای صفحه ↑"
              />
            </button>
          </div>
        </div>

        {/* AH∞RA Brand Logo & Credit - Static & Non-editable by admin */}
        <div className="mt-5 sm:mt-6 pt-3 flex items-center justify-center">
          <a
            ...
            className={`group inline-flex items-center justify-center gap-1.5 sm:gap-2 py-0.5 sm:py-1 px-2 sm:px-2.5 transition-all duration-300 select-none ${
              isEditorMode ? 'cursor-default' : 'cursor-pointer'
            }`}
            title="شرکت نوآوران هوشمند اهورا (طراحی و توسعه)"
          >
            ...
            {/* 2. متن طراحی و توسعه */}
            <span className="text-[10px] sm:text-xs text-[#EDEAE4]/55 group-hover:text-[#EDEAE4]/80 transition-colors font-vazir font-normal inline-flex items-center gap-1.5 flex-wrap justify-center">
              <span>{creditPrefixText}</span>
