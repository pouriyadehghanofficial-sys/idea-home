// Cloudflare Pages Function: /api/price-list/download
// Proxies an ImageKit-hosted price list file through our own domain, so users
// whose network filters or corporate firewalls restrict direct access can still download it.
//
// USAGE (from frontend):
//   /api/price-list/download?url=<encoded ImageKit URL>
//
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const guessedName = decodeURIComponent(parsed.pathname.split('/').pop() || 'IdeaHome-PriceList.pdf');
    ...
    return new Response(
      JSON.stringify({ success: false, message: err.message || 'خطای سرور پروکسی دانلود' }),
